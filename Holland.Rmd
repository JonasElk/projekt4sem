...
```{r}
data = read_excel("data/data.xlsx", sheet = "NET")
data = data[-c(1:11),]
#data = data[-c(50:52),]
```
...
summary(dfm)
dfm_robust <- coeftest(dfm, vcov = vcovHC(dfm, type = "HC0"))
screenreg(list(OLS = dfm, OLS_robust_se = dfm_robust), digits = 4)


dfm1 = dynlm(d(C_l) ~ d(R_l) + d(W_l) + lead(d(W_l)))
summary(dfm1)
dfm1_robust <- coeftest(dfm1, vcov = vcovHC(dfm1, type = "HC0"))
screenreg(list(OLS = dfm1, OLS_robust_se = dfm1_robust), digits = 4)
